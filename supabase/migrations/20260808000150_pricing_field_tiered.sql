-- Migración 150: pricing dinámico por campo del form. Normaliza reglas eval/instalación a {field,tiers} + extiende
-- _public_price_order con field_tiered (evaluationType/installQty) + cupón promo primer-ciclo/recurrente + steps trace.
-- rule_type se queda en 'tiered_qty' (el CHECK no tiene 'field_tiered'); la lógica vive en config. Anti-tamper intacto.

-- Normaliza reglas a {field, tiers:[{value,price}]} (valores = los del select del form)
update public.tenant_pricing_rules set config = '{"field":"evaluationType","tiers":[{"value":"individual","price":80},{"value":"condominio","price":200},{"value":"urb-pequena","price":260},{"value":"urb-grande","price":500}]}'::jsonb
  where id = '22222222-0000-4000-a000-000000000003';
update public.tenant_pricing_rules set config = '{"field":"installQty","tiers":[{"value":"1","price":495},{"value":"2","price":990},{"value":"3","price":1395},{"value":"4","price":1850}]}'::jsonb
  where id = '22222222-0000-4000-a000-000000000006';

-- Drop del 4-arg (evita overload que mantendría la versión sin field_tiered) + nuevo con _is_first_cycle
drop function if exists public._public_price_order(uuid, jsonb, jsonb, text);
...
declare _base numeric := 0; _tax numeric := 0; _ship numeric := 0; _disc numeric := 0; _taxpct numeric;
        _it jsonb; _kind text; _iid uuid; _qty numeric; _p numeric; _mrule jsonb; _arule jsonb; _trule jsonb; _promo jsonb; _c record;
...
begin
  ...
  for _it in select value from jsonb_array_elements(coalesce(_items,'[]'::jsonb)) loop
    ...
    elsif _kind='service' and _freq is not null and _mrule is not null then
  ...
  return jsonb_build_object('subtotal',round(_base,2),'tax',_tax,'shipping',_ship,'discount',_disc,'total',round(_base+_tax+_ship-_disc,2),
    'breakdown',jsonb_build_object('steps',_steps,'tax_pct',coalesce(_taxpct,0),'coupon_applied',_disc>0,'matrix_used',_freq is not null and _mrule is not null and _trule is null));
end $fn$;
