-- 20260706000041_reconciliation_snapshot_v2.sql
-- Conciliación v2: snapshot con conciliación real de banco (inicial+depósitos-egresos vs real),
-- retención mensual del año, desglose de gastos e indicadores de salud (break-even, margen, tendencia).
-- Delega en tax_obligations_for + expense_breakdown_for + monthly_series_for.
...
returns jsonb language sql stable security definer set search_path = public as $$
  ...
  s as (
    select t.*,
      ...
      (select jsonb_agg(jsonb_build_object('bankName',ba.bank_name,'openingBalance',r.opening_balance,'realBalance',r.real_balance,'cutoffDate',r.cutoff_date) order by ba.bank_name)
         from public.bank_accounts ba join public.bank_balance_records r on r.bank_account_id=ba.id
         where ba.tenant_id=t.tid and r.period_year=t.py and r.period_month=t.pm) as accts,
