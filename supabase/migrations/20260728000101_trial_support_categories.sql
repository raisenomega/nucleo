-- 20260728000101_trial_support_categories.sql
-- Añade el seed de categorías de soporte (kind='support_category') a create_trial_tenant (tenants NUEVOS).
-- Idéntico a la migr 78 salvo el bloque de categorías, que ahora incluye las 5 de soporte.
...
begin
  ...
  insert into public.tenants (slug, legal_name, status, expires_at)
    values (_slug, business_name, 'trial', now() + interval '3 days') returning id into _tenant;
