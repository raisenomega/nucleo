-- 20260706000049_reconciliation_snapshot_v4.sql
-- Sub-slice C: break-even = solo gastos FIJOS + nómina (no todos los egresos). Egresos de nómina
-- usan total_employer_cost (decisión 4). Panel 4 desglosado por expense_class (expense_classes_for).
...
returns jsonb language sql stable security definer set search_path = public as $$
  ...
  c as (select *, exp+pay+ext+mkt as out, (tax->>'total_estimated')::numeric as taxest, round(inc*rpct/100,2) as req,
    (ec->>'fixed')::numeric as efixed from s),
  b as (select *, efixed+pay as beven from c)
  select jsonb_build_object(
    ...
    'summary_panel', jsonb_build_object('total_income', inc, 'total_expenses', exp, 'total_payroll', pay,
       ...
       'health', jsonb_build_object('total_out', out, 'break_even', beven,
         'fixed_expenses', efixed, 'variable_expenses', (ec->>'variable')::numeric,
