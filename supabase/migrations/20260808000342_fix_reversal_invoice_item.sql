-- REVERSAL-INVOICE-ITEM-FIX (MONEY-CRITICAL): la reversión NO cobraba dinero real.
-- CAUSA: el código creaba el invoice_item como PENDING (sin invoice) y luego POST /v1/invoices esperando que la
-- invoice lo absorbiera. En la API moderna de Stripe la invoice NO incluye los pending items → salía en $0.00,
-- auto_advance la finalizaba y la marcaba "pagada", y el item quedaba HUÉRFANO (verificado: ii_1TzM2z… $20.01
-- con invoice=null). Peor: el POST del item usaba `perform http(...)` → DESCARTABA el status, y el audit_log
-- registraba 'subscription_reversal_applied' con invoice_status=200 (que solo probaba que la invoice se creó)
-- = FALSE-POSITIVE. Protección DACO inoperante: el cliente que cancelaba antes del compromiso no pagaba nada.
-- FIX: (1) crear invoice draft (auto_advance=false) → (2) crear el invoice_item ADJUNTO a esa invoice
-- (`invoice=<id>`, determinista) → (3) finalize → (4) pay; se verifica el status de CADA llamada y el audit
-- refleja el RESULTADO REAL (amount_paid): solo marca reversal_applied si cobró > 0, si no registra failed.
...
declare _oid uuid := nullif(p_sub->'metadata'->>'order_id','')::uuid; _o record;
        ...
        _rev bigint; _inv text; _paid numeric := 0; _sk text; _name text; _cust text := p_sub->>'customer'; _st int; _resp text;
begin
    ...
    if _sk is not null then
      begin
        ...
        select status, content into _st, _resp from http(('POST','https://api.stripe.com/v1/invoices/'||_inv||'/pay',
          array[http_header('Authorization','Bearer '||_sk)],'application/x-www-form-urlencoded','')::http_request);
