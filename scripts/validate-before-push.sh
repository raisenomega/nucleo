...
# - sentinel_scans / hermes_checkpoints: registros globales de PLATAFORMA (SENTINEL/HERMES, superadmin-only, sin tenant).
# Exenciones legítimas a la regla tenant_id obligatorio:
# 1. Tablas globales-por-diseño: tenants, categories, settings, tenant_order_counters, rate_limit_public, sentinel_scans, hermes_checkpoints.
# 2. Sub-estructuras que heredan tenant_id del parent (partition of).
section "8. Migraciones con tabla sin tenant_id:"
NO_TENANT=""
if [ -d "$MIG_DIR" ]; then
  while IFS= read -r f; do
    if grep -qiE "create table[[:space:]]+(if not exists[[:space:]]+)?(public\.)?(tenants|categories|settings|tenant_order_counters|rate_limit_public|sentinel_scans|hermes_checkpoints|marketing_[a-z_]+)" "$f" || grep -qiE "partition of" "$f"; then
      continue
    fi
