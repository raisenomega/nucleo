...
import { signInCustomer, magicLinkCustomer, resetPasswordCustomer } from "@shared/portal/portal-auth";
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-background p-4 text-foreground">
      <form onSubmit={login} className="w-full max-w-sm space-y-4">
        {logoUrl ? <img src={logoUrl} alt={name} className="mx-auto max-h-16 w-auto" /> : <h1 className="text-center font-display text-2xl font-bold">{name}</h1>}
        <p className="text-center text-sm text-muted-foreground">{t("pLoginSubtitle")}</p>
        ...
        <button type="button" onClick={() => void forgot()} className="w-full text-center text-xs text-muted-foreground underline">{t("pForgot")}</button>
