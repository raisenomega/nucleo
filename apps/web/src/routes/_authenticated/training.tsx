...
import { ResourceLibrary } from "@hr/presentation/ResourceLibrary";
...
type Mat = { id: string; title: string };
...
function TrainingPage() {
  ...
  const [tab, setTab] = useState<"courses" | "assign" | "library">("courses");
    ...
    <div className="space-y-6 p-4 md:p-8">
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-4">
          ...
          {can("training", "create") && tab !== "library" && <button type="button" onClick={() => setCreating((v) => !v)}
      ...
      <div className="flex gap-2 border-b border-border">
        ...
        <button type="button" onClick={() => { setTab("library"); setCreating(false); }} className={tabCls("library")}>{t("library")}</button>
      </div>
      {tab === "courses" ? (
        <>{creating && <CourseForm onSubmit={m.saveCourse} onCancel={() => setCreating(false)} />}
          <CourseTable rows={m.courses} onMaterials={(c) => setMat({ id: c.id, title: c.title })}
            onDelete={canEdit ? (id) => { if (window.confirm(`${t("delete")}?`)) void m.removeCourse(id); } : undefined} /></>
      ) : tab === "assign" ? (
        <>{creating && <AssignForm employees={emps} courses={m.courses} onSubmit={m.assign} onCancel={() => setCreating(false)} />}
          <EnrollmentTable rows={m.enrollments} onComplete={canEdit ? complete : undefined} onMaterials={(id, title) => setMat({ id, title })}
            onDelete={canEdit ? (id) => { if (window.confirm(`${t("delete")}?`)) void m.removeEnrollment(id); } : undefined} /></>
      ) : <ResourceLibrary canEdit={canEdit} tenantId={session?.tenantId ?? ""} />}
      {mat && <CourseResourcesPanel courseId={mat.id} courseTitle={mat.title} canEdit={canEdit} onClose={() => setMat(null)} />}
    </div>
  );
}
