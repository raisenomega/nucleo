    ...
    <div className="space-y-6 p-4 md:p-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {can("payroll", "create") && <button type="button" onClick={() => setManaging("new")} className="flex items-center gap-2 rounded-lg bg-secondary text-foreground px-3 py-2 text-sm font-body font-bold"><Users className="h-4 w-4" /> {t("externalWorkers")}</button>}
      ...
      <ExternalWorkersTable rows={workers.items} onAdd={() => setManaging("new")} onEdit={(id) => setManaging(id)} onToggle={(w) => void workers.update(w.id, { ...w, active: !w.active })} />
