...
import { CyclesTab } from "@hr/presentation/CyclesTab";
...
function EvaluationsPage() {
  ...
  const [tab, setTab] = useState<"evals" | "feedback" | "cycles">("evals");
    ...
    <div className="space-y-6 p-4 md:p-8">
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-4">
          ...
          {tab !== "cycles" && can("evaluations", "create") && <button type="button" onClick={() => setCreating((v) => !v)}
      ...
      <div className="flex gap-2 border-b border-border">
        ...
        {canCoo && <button type="button" onClick={() => { setTab("cycles"); setCreating(false); }} className={tabCls("cycles")}>{t("evalCycles")}</button>}
      </div>
      {tab === "evals" ? (
      ...
      ) : <CyclesTab criteria={ev.criteria} />}
    </div>
  );
}
