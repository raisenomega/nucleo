...
export interface SaleLine { description: string; quantity: number; unitPrice: number; lineTotal: number; taxPct?: number; discountPct?: number; productId?: string | null; sku?: string | null; }
        ...
        return (
          <Row key={i} type={clickable ? "button" : undefined} onClick={clickable ? () => onLineClick(it.productId as string) : undefined}
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground">{it.quantity} × {formatCurrency(it.unitPrice)}{it.discountPct ? ` · desc ${it.discountPct}%` : ""}{it.taxPct ? ` · IVU ${it.taxPct}%` : ""}</p>
            </div>
