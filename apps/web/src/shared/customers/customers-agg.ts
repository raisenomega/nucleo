...
// Deuda = solo facturas EMITIDas y abiertas (draft no es AR — rodaja 11 Bug #1); suma el balance, no el total.
const OWES = (st: string) => st === "sent" || st === "partially_paid" || st === "overdue";
...
export function aggregate(customers: CustomerBase[], orders: OrderLite[], reviews: ReviewLite[], invoices: InvoiceLite[], now: number): { rows: AdminCustomer[]; kpis: CustomerKpis } {
  const rows: AdminCustomer[] = customers.map((c) => {
    ...
    return {
      ...
      debt: iv.filter((i) => OWES(i.status)).reduce((sm, i) => sm + i.balance, 0),
    };
  });
