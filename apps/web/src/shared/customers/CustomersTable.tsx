...
export function CustomersTable({ rows, onView }: { rows: readonly AdminCustomer[]; onView: (id: string) => void }) {
  const { t } = useI18n();
      ...
      <div className="hidden overflow-hidden rounded-lg border border-border bg-card md:block"><div className="overflow-x-auto"><table className="w-full text-sm">
        ...
        <tbody>{visible.map((c) => (
          <tr key={c.id} onClick={() => onView(c.id)} className="cursor-pointer border-t border-border hover:bg-secondary">
            <td className="px-3 py-2 font-medium">{c.fullName || "—"}</td><td className="px-3 py-2 text-muted-foreground">{c.email}</td><td className="px-3 py-2 text-muted-foreground">{c.phone || "—"}</td>
