import type { TranslationKey } from "./translations.keys";

// Diccionario suplementario (RRHH / evaluaciones). Se fusiona en translations.ts.
export const esHr = {
  evaluationsSubtitle: "Scoring de desempeño ponderado + compliance PR (Ley 80)",
  newEvaluation: "Nueva evaluación", autoSuggest: "Auto-sugerir desde datos",
  composite: "Score compuesto", classification: "Clasificación",
  clExcelente: "Excelente", clBueno: "Bueno", clNecesitaMejora: "Necesita mejora", clInsuficiente: "Insuficiente",
  legalWarning: "Requiere validación legal (Ley 80): despido con causa documentada.",
  observations: "Observaciones", observationsSubtitle: "Bitácora de coaching e incidentes por empleado",
  newObservation: "Nueva observación", followUp: "Fecha de seguimiento", recentObservations: "Observaciones recientes",
  obsLogro: "Logro", obsMejora: "Oportunidad de mejora", obsIncidente: "Incidente",
  obsCultural: "Comportamiento cultural", obsDesarrollo: "Sugerencia de desarrollo",
  anonymous: "Anónimo", author: "Autor", acknowledge: "Acusar recibo",
  evTopDown: "Evaluación formal", evPeer: "Peer review", evBottomUp: "Bottom-up (a mi jefe)", evSelf: "Auto-evaluación",
  fbTab: "Feedback", newFeedback: "Nueva opinión", fbGeneral: "Empresa (general)", fbContent: "Escribe tu opinión…",
  fbSuggestion: "Sugerencia", fbPraise: "Reconocimiento", fbConcern: "Preocupación", fbCulture: "Cultura", fbTip: "Anónimo",
  currentPeriod: "Período actual", freqWeekly: "Semanal", freqBiweekly: "Quincenal", freqMonthly: "Mensual",
  periodFeedback: "Feedback del período",
  trainingSubtitle: "Cursos + asignaciones + cumplimiento", courses: "Cursos", assignments: "Asignaciones",
  hr: "RRHH", signature: "Firma", clearSignature: "Limpiar firma", validityMonths: "Validez (meses)",
  calcFromAttendance: "Calcular desde asistencia", applyGross: "Aplicar", newCycle: "Nuevo ciclo", evalCycle: "Ciclo", evalCycles: "Ciclos",
  activateCycle: "Activar", ct_monthly: "Mensual", ct_quarterly: "Trimestral", ct_semi_annual: "Semestral", ct_annual: "Anual", ct_custom: "Personalizado",
  cs_draft: "Borrador", cs_active: "Activo", cs_closed: "Cerrado", cs_cancelled: "Cancelado",
  periodStart: "Inicio período", periodEnd: "Fin período", evalStart: "Inicio evaluación", evaluator: "Evaluador", evalType: "Tipo",
  newCourse: "Nuevo curso", assignCourse: "Asignar curso", courseTitle: "Título del curso", hours: "Horas",
  required: "Obligatorio", completion: "Cumplimiento", markComplete: "Marcar completado", course: "Curso", trainingPct: "Capacitación",
  stNotStarted: "Sin iniciar", stInProgress: "En progreso", stCompleted: "Completado", stExpired: "Vencido",
  library: "Biblioteca", document: "Documento", video: "Video", presentation: "Presentación", image: "Imagen", link: "Enlace",
  addResource: "Nuevo recurso", requiredMaterial: "Obligatorio", dragOrClick: "Arrastra o haz clic para subir", maxFileSize: "Máx. 50MB",
  videoPreview: "Vista previa", urlNotRecognized: "URL de video no reconocida — se mostrará como enlace", courseMaterials: "Materiales del curso",
  addMaterial: "Añadir material", viewResource: "Ver recurso", unlinkFromCourse: "Desvincular", visibleToEmployees: "Visible para empleados",
  openLink: "Abrir enlace", uploadFile: "Subir archivo", close: "Cerrar", download: "Descargar",
  myPortal: "Mi portal", employeePortal: "Portal del empleado", myProfile: "Mi perfil", myPayroll: "Mi nómina",
  myEvaluations: "Mis evaluaciones", myLeave: "Mis vacaciones", myTraining: "Mi capacitación", myAttendance: "Mi asistencia",
  myOnboarding: "Mi onboarding", overview: "Resumen", greeting: "Hola", pendingItems: "Pendientes", lastPayroll: "Última nómina",
  availableDays: "Días disponibles", completedCourses: "Cursos completados", payStub: "Recibo de pago", deductions: "Deducciones",
  downloadPayStub: "Descargar recibo", annualSummary: "Resumen anual", annualGross: "Bruto anual", annualNet: "Neto anual",
  evaluationPending: "evaluación(es) por completar", certExpiring: "certificación(es) por vencer", viewMaterials: "Ver materiales",
  startCourse: "Empezar", continueCourse: "Continuar", jobInfo: "Datos del puesto", completeEvaluation: "Completar evaluación",
  myCertifications: "Mis certificaciones", certValid: "Vigente", noPending: "Nada pendiente 🎉", issued: "Emitida",
  supportSubtitle: "Tickets internos + comentarios + adjuntos", newTicket: "Nuevo ticket", subject: "Asunto",
  priority: "Prioridad", assignedTo: "Asignado a", unassigned: "Sin asignar", takeTicket: "Tomar ticket",
  comments: "Comentarios", attachment: "Adjunto",
  prioLow: "Baja", prioNormal: "Normal", prioHigh: "Alta", prioUrgent: "Urgente",
  addComment: "Escribir comentario…",
  tkOpen: "Abierto", tkInProgress: "En progreso", tkResolved: "Resuelto", tkClosed: "Cerrado",
} satisfies Partial<Record<TranslationKey, string>>;
