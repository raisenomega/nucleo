  ...
  useEffect(() => {
    ...
    void (async () => {
      const [ten, theme, files] = await Promise.all([
        supabase.from("tenants").select("legal_name,display_name,landing_enabled,gl_enabled").limit(1),
        supabase.from("tenant_themes").select("*").limit(1),
        supabase.storage.from("brand").list(tenantId),
      ]);
      if (!alive) return;
      const row = (ten.data as { legal_name: string; display_name: string | null; landing_enabled?: boolean; gl_enabled?: boolean }[] | null)?.[0];
      ...
      setBrand({
        ...
        isLoading: false, landingEnabled: row?.landing_enabled ?? false, glEnabled: row?.gl_enabled ?? false, reload,
      });
    })();
    return () => { alive = false; };
