  ...
  useEffect(() => {
    ...
    void (async () => {
      const [ten, theme, files] = await Promise.all([
        supabase.from("tenants").select("legal_name,display_name,landing_enabled,gl_enabled,gps_realtime_enabled,fulfillment_enabled,is_demo_tenant").limit(1),
        supabase.from("tenant_themes").select("*").limit(1),
        supabase.storage.from("brand").list(tenantId),
      ]);
      if (!alive) return;
      const row = (ten.data as { legal_name: string; display_name: string | null; landing_enabled?: boolean; gl_enabled?: boolean; gps_realtime_enabled?: boolean; fulfillment_enabled?: boolean; is_demo_tenant?: boolean }[] | null)?.[0];
      ...
      setBrand({
        ...
        gpsRealtimeEnabled: row?.gps_realtime_enabled ?? false, fulfillmentEnabled: row?.fulfillment_enabled ?? false,
        isDemoTenant: row?.is_demo_tenant ?? false, reload,
      });
    })();
    return () => { alive = false; };
