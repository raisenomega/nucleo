...
const ISEL = "id, invoice_number, status, total, amount_paid, balance, due_date, paid_at, pdf_url, linked_order_id";
...
const toInvoice = (r: Row): CustomerInvoice => ({ id: r.id as string, invoiceNumber: (r.invoice_number as string) ?? "", status: (r.status as string) ?? "", total: n(r.total), amountPaid: n(r.amount_paid), balance: r.balance == null ? n(r.total) : n(r.balance), dueDate: (r.due_date as string) ?? null, paidAt: (r.paid_at as string) ?? null, pdfUrl: (r.pdf_url as string) ?? "", linkedOrderId: (r.linked_order_id as string) ?? null });
