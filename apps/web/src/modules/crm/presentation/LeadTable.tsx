              ...
              <tr key={l.id} onClick={() => onView(l.id)} className="cursor-pointer border-t border-border transition-colors hover:bg-secondary">
                ...
                <td className="px-3 py-2" onClick={(e) => e.stopPropagation()}>
                  <div className="flex justify-end gap-2">
                    ...
                    {docs && <button type="button" onClick={() => void leadQuoteId(l.id).then((r) => { if (r.ok) nav({ to: "/quotes" }); else window.alert(r.error); })} aria-label={t("quote")} className="text-foreground"><FileText className="h-4 w-4" /></button>}
                    {docs && <button type="button" onClick={() => void leadInvoiceId(l.id).then((r) => { if (r.ok) nav({ to: "/billing" }); else window.alert(r.error); })} aria-label={t("invoice")} className="text-foreground"><Receipt className="h-4 w-4" /></button>}
