...
export function MarketingExpenseForm({ initial, onSubmit, onCancel }: {
  initial: MExpenseFormData; onSubmit: (d: MExpenseFormData) => void; onCancel: () => void;
}) {
  const { t } = useI18n();
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(f); }} className="space-y-4 rounded-lg border border-border bg-card p-5">
      <h2 className="font-body font-bold">{t("newMExpense")} · {f.channel}</h2>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <label className="space-y-1 md:col-span-3"><span className={lbl}>{t("description")}</span>
