...
export function CheckoutModal({ assetName, profiles, onSubmit, onClose }: {
  ...
  const [err, setErr] = useState<"" | "emp" | "odo">("");
  ...
  const bad = (k: "emp" | "odo") => (err === k ? fld.replace("border-border", "border-destructive") : fld);
  // Validación VISIBLE (toast + borde rojo) en vez del required nativo (silencioso en móvil, #checkin-ux).
  const go = (e: React.FormEvent) => { e.preventDefault(); if (!f.employeeId) { setErr("emp"); return void toast.error(t("selectEmployee")); } if (f.odometer == null) { setErr("odo"); return void toast.error(t("enterOdometer")); } onSubmit(f); };
  return (
    <ScreenModal onClose={onClose}>
      ...
      <form onSubmit={go} className="grid grid-cols-1 gap-3 p-4 md:grid-cols-2">
        <label className="space-y-1"><span className={lbl}>{t("employee")}</span><select value={f.employeeId} onChange={(e) => { setF({ ...f, employeeId: e.target.value }); if (e.target.value) setErr(""); }} className={bad("emp")}><option value="">—</option>{profiles.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
        <label className="space-y-1"><span className={lbl}>{t("odometer")}</span><input type="number" min="0" value={f.odometer ?? ""} onChange={(e) => { const v = e.target.value ? Number(e.target.value) : null; setF({ ...f, odometer: v }); if (v != null) setErr(""); }} className={bad("odo")} /></label>
