...
const line = (m: QuoteItem) => m.quantity * m.unitPrice * (1 - m.discountPct / 100) * (1 + m.taxPct / 100);

export function QuoteItemsEditor({ items, onChange }: {
  items: readonly QuoteItem[]; onChange: (items: QuoteItem[]) => void;
}) {
  const { t } = useI18n();
  const total = items.reduce((s, i) => s + i.lineTotal, 0);
  const inp = "w-full rounded border border-border bg-background p-1 text-sm";
  function set(idx: number, k: keyof QuoteItem, v: string) {
    onChange(items.map((it, i) => {
      if (i !== idx) return it;
      const m = { ...it, [k]: k === "description" ? v : Number(v) } as QuoteItem;
