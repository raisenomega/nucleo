...
const toConfig = (r: Row): LandingConfig => ({
  ...
  catalogPageTitle: (r.catalog_page_title as string) ?? "", catalogPageSubtitle: (r.catalog_page_subtitle as string) ?? "",
});
const fromConfig = (tenantId: string, c: LandingConfig) => ({
  ...
  catalog_page_title: c.catalogPageTitle || null, catalog_page_subtitle: c.catalogPageSubtitle || null,
});
