...
export function ProductBasicInfoSection({ form, set, categories }: SectionProps & { categories: LandingCategory[] }) {
  const { t } = useI18n();
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-3">
        <label className="block"><span className={lbl}>{t("sku")}</span>
          <input value={form.sku} onChange={(e) => set("sku", e.target.value)} className={`w-40 ${fld}`} /></label>
        ...
        <label className="flex items-center gap-2 pb-2 text-sm"><input type="checkbox" checked={form.isActive} onChange={(e) => set("isActive", e.target.checked)} /> {t("active")}</label>
      </div>
