import { PackagePlus, SlidersHorizontal, PackageMinus, ArrowLeftRight, Pencil, Trash2 } from "lucide-react";
import { useI18n } from "@shared/i18n";
import { useModuleAccess } from "@shared/hooks/useModuleAccess";

// Acciones por fila de inventario (extraído para mantener InventoryTable ≤75). Reabastecer/ajustar/merma/transferir/editar/borrar.
export function InventoryRowActions({ id, onRestock, onAdjust, onShrink, onTransfer, onEdit, onDelete }: {
  id: string; onRestock: (id: string) => void; onAdjust: (id: string) => void; onShrink: (id: string) => void;
  onTransfer: (id: string) => void; onEdit: (id: string) => void; onDelete: (id: string) => void;
}) {
  const { t } = useI18n();
