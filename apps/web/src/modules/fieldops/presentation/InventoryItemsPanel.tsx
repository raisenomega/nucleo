...
export function InventoryItemsPanel({ inv, rows, movs, now, suppliers, slow, high, reorder, focusId, onEdit, onDelete }: {
  ...
  async function doAction(qty: number, reason: string, warehouseId: string | null, lotId: string | null) { if (!action) return; const r = action.mode === "adjust" ? await inv.adjust(action.id, qty, reason, warehouseId, lotId) : await inv.shrink(action.id, qty, reason, warehouseId, lotId); if (r.ok) { setAction(null); toast.success(t("saved")); } else toast.error(r.error); }
