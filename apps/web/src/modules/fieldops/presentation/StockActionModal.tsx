...
import { LotPicker } from "@fieldops/presentation/LotPicker";
import type { InventoryItem } from "@fieldops/domain/inventory.types";

// FIX6 — ajuste/merma POR ALMACÉN. Con trazabilidad, la operación es sobre un LOTE específico.
export function StockActionModal({ item, mode, onSubmit, onClose }: {
  item: InventoryItem; mode: "adjust" | "shrink"; onSubmit: (qty: number, reason: string, warehouseId: string | null, lotId: string | null) => void; onClose: () => void;
}) {
  const { t } = useI18n();
  ...
  useEffect(() => { setQty(mode === "adjust" && tracking === "none" ? whStock : 0); }, [wh]);
  const field = "w-full rounded-lg border border-border bg-background p-2 font-body";
  const lbl = "text-xs font-bold text-muted-foreground";
  const go = (e: React.FormEvent) => { e.preventDefault(); if (mode === "shrink" && qty <= 0) return; if (tracking !== "none" && !lot) return; onSubmit(qty, reason, wh || null, lot || null); };
  return (
    <ScreenModal onClose={onClose}>
      ...
      <form onSubmit={go} className="space-y-3 p-4">
        ...
        {tracking !== "none" && <LotPicker itemId={item.id} warehouseId={wh || undefined} value={lot} onChange={setLot} />}
        {tracking === "none" && <p className="text-sm text-muted-foreground">{t("stock")}: <b className="text-foreground">{whStock}{uom}</b></p>}
