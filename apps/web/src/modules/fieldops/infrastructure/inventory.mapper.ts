...
export interface Row {
  id: string; tenant_id: string; name: string; stock: number | string; unit_cost: number | string; min_stock: number | string;
  ...
  category_id: string | null; category: { id: string; label: string } | null; unit_of_measure_id: string | null; unit_of_measure: { id: string; name: string; abbreviation: string } | null; barcode: string | null; stock_entries: StockRow[] | null;
}
...
export const SELECT = "id, tenant_id, name, stock, unit_cost, min_stock, sku, avg_cost, supplier_name, supplier_id, landing_product_id, last_restock_date, warehouse_zone, aisle, shelf, bin, reorder_point, reorder_qty, photo_urls, category_id, category:categories!category_id(id, label), unit_of_measure_id, unit_of_measure:units_of_measure!unit_of_measure_id(id, name, abbreviation), barcode, stock_entries:inventory_stock(warehouse_id, quantity, min_stock, reorder_point, reorder_qty, location_zone, location_aisle, location_shelf, location_bin, warehouse:warehouses(id, name, code))";

export function toItem(r: Row): InventoryItem {
  return {
    id: r.id, tenantId: r.tenant_id, name: r.name, stock: Number(r.stock), unitCost: Number(r.unit_cost), minStock: Number(r.min_stock), sku: r.sku ?? "",
