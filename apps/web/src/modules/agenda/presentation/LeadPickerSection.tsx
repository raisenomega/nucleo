...
export function LeadPickerSection({ leadId, leadName, onPick }: { leadId: string | null; leadName: string; onPick: (id: string | null, name: string) => void }) {
  const { t } = useI18n();
  ...
  async function doCreate() { const l = await create(nn, np, ne); if (l) { onPick(l.id, l.name); setCreating(false); } }
  ...
  return (
    <div className="space-y-2">
      ...
      {results.length > 0 && <ul className="rounded-lg border border-border">{results.map((r) => <li key={r.id}><button type="button" onClick={() => onPick(r.id, r.name)} className="w-full p-2 text-left text-sm hover:bg-secondary">{r.name}</button></li>)}</ul>}
      {!creating ? <button type="button" onClick={() => setCreating(true)} className="text-sm text-primary">+ {t("agendaNewLead")}</button>
