...
export function useAgendaController() {
  ...
  async function reschedule(id: string, startMs: number) {
    ...
    const base: AppointmentInput = { leadId: a.leadId, serviceId: a.serviceId, title: a.title, status: a.status, notes: a.notes, startsAt: new Date(startMs).toISOString(), endsAt: new Date(endMs).toISOString() };
