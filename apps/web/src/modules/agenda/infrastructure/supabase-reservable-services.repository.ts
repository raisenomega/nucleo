...
import type { IReservableServicesRepository, Result } from "@agenda/domain/reservable-service.types";
...
export const supabaseReservableServicesRepository: IReservableServicesRepository = {
  async list() {
    const { data } = await supabase.from("tenant_landing_services")
      .select("id,name,duration_estimate_minutes,reserve_type,reserve_price").eq("requires_scheduling", true).order("name");
    return ((data ?? []) as Row[]).map((r) => ({
      id: r.id, name: r.name, durationMinutes: r.duration_estimate_minutes,
      reserveType: r.reserve_type ?? "none", reservePrice: r.reserve_price != null ? Number(r.reserve_price) : null,
    }));
  },
