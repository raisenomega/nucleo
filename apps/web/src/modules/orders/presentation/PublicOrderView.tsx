...
export function PublicOrderView({ data, paid, subscribed }: { data: PublicOrderResp; paid?: boolean; subscribed?: boolean }) {
  const { t } = useI18n();
  ...
  return (
    <main className="min-h-screen bg-background p-4 text-foreground">
      <div className="mx-auto max-w-2xl space-y-4">
        {(paid || subscribed) && <div className="rounded-lg border border-green-600/40 bg-green-600/10 p-3 text-center text-sm font-bold text-green-700">✅ {t(subscribed ? "subActivated" : "payReceived")}</div>}
