...
const SEL = "id,order_number,customer_name,customer_email,customer_phone,items,subtotal,tax,shipping,discount,total,currency,status,source_hostname,utm_source,utm_medium,utm_campaign,linked_lead_id,linked_invoice_id,created_at,paid_at";
...
export const supabaseOrdersRepository: IOrdersRepository = {
  ...
  async confirm(id: string, paymentMethodId: string | null, createInvoice: boolean): Promise<Result> {
    const { data } = await supabase.rpc("confirm_landing_order", { _order_id: id, _payment_method_id: paymentMethodId, _create_invoice: createInvoice });
    const d = data as { status?: string; code?: string } | null;
    return d?.status === "ok" ? { ok: true } : { ok: false, error: errMsg(d?.code) };
  },
