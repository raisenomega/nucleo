...
// toggle ES/EN, CTA dorado "Solicitar acceso". Todo con scroll-a-sección (single-page, sin router).
export function MarketingNav({ lang, toggleLang }: { lang: Lang; toggleLang: () => void }) {
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-50 flex items-center justify-between bg-background/40 px-6 py-4 backdrop-blur-sm md:px-12">
      <button type="button" onClick={() => scrollTo("hero")} className="font-display text-lg font-bold tracking-tight text-white">
      ...
      <div className="flex items-center gap-3">
        <button type="button" onClick={toggleLang} aria-label="idioma" className="rounded-full border border-white/20 px-3 py-1 text-xs font-semibold text-white/70 transition-colors hover:text-white">{lang === "es" ? "EN" : "ES"}</button>
        <button type="button" onClick={() => scrollTo("lead-form")} className="rounded-full bg-primary px-4 py-1.5 text-xs font-semibold text-primary-foreground transition-transform hover:scale-105">{c.navCta}</button>
      </div>
    </header>
  );
}
