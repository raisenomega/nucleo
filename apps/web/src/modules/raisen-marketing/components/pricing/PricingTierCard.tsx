...
// features con Check. CTA (label+href editables) → scroll al ancla. Dorado = token primary (tema .rm-root).
  ...
  return (
    <div className={`relative flex flex-col rounded-xl border p-6 transition-all duration-300 ${tier.isRecommended ? "border-primary/40 bg-card shadow-lg shadow-primary/10" : "border-border bg-card hover:border-primary/20"}`}>
      ...
      <p className="mt-4 font-display text-foreground">
        ...
        <span className="ml-1 text-sm font-normal text-muted-foreground">{tier.currency}{period}</span>
      </p>
      ...
      <button type="button" onClick={() => goTo(tier.ctaHref)} className="mt-6 w-full rounded-lg bg-primary py-3 text-center text-sm font-semibold text-primary-foreground transition-transform hover:scale-[1.02]">{es ? tier.ctaLabelEs : tier.ctaLabelEn}</button>
    </div>
  );
}
