...
// Las 6 primeras usan el LOGO REAL de marca (SVG propio); el resto cae a un ícono lucide del set compartido.
const SUGGEST: Record<string, string> = { Facebook: "facebook", Instagram: "instagram", YouTube: "youtube", LinkedIn: "linkedin", WhatsApp: "whatsapp", TikTok: "tiktok", X: "AtSign", GitHub: "Code", Website: "Globe" };

// Crear/editar red del footer. Elegir una plataforma común asigna su logo real; para otras, LucideIconPicker.
export function SocialLinkDialog({ initial, onClose, onSave }: { initial: SocialLink | null; onClose: () => void; onSave: (s: SocialLinkDraft) => void }) {
  ...
  const Preview = socialIcon(d.iconName, d.platform);
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" onClick={onClose}>
      <div className="max-h-[85vh] w-full max-w-md space-y-3 overflow-y-auto rounded-xl border border-border bg-card p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-3">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary"><Preview size={20} /></span>
          <input className={F} placeholder="Plataforma (ej. Instagram)" value={d.platform} onChange={(e) => set({ platform: e.target.value })} />
        </div>
        <div className="flex flex-wrap gap-1">
          {Object.keys(SUGGEST).map((p) => <button key={p} type="button" onClick={() => set({ platform: p, iconName: SUGGEST[p]! })} className="rounded-full border border-border px-2 py-0.5 text-xs text-muted-foreground hover:text-foreground">{p}</button>)}
        </div>
        <input className={F} placeholder="https://…" value={d.url} onChange={(e) => set({ url: e.target.value })} />
        <details><summary className="cursor-pointer text-xs text-muted-foreground">Ícono personalizado (otras plataformas)</summary>
          <div className="pt-2"><LucideIconPicker value={d.iconName} onChange={(iconName) => set({ iconName })} /></div>
        </details>
