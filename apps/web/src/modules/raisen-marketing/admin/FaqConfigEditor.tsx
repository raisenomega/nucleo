...
import { saveFaqConfig } from "@raisen-marketing/infrastructure/marketing-faq.repository";
import type { FaqConfig } from "@raisen-marketing/data/faq.types";

const F = "w-full rounded-lg border border-border bg-background p-2 text-sm text-foreground";

// Editor del encabezado de la sección FAQ (eyebrow + título ES/EN).
export function FaqConfigEditor({ config }: { config: FaqConfig }) {
  const toast = useToast();
  const [c, setC] = useState(config);
  const set = (p: Partial<FaqConfig>) => setC((x) => ({ ...x, ...p }));
  const save = async () => { const e = await saveFaqConfig(c); if (e) toast.error(e); else toast.success("Config guardada"); };
  return (
