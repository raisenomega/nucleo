...
import { savePricingConfig } from "@raisen-marketing/infrastructure/marketing-pricing.repository";
import type { PricingConfig } from "@raisen-marketing/data/pricing.types";

const F = "w-full rounded-lg border border-border bg-background p-2 text-sm text-foreground";

// Editor del encabezado de la sección Precios (eyebrow + título ES/EN).
export function PricingConfigEditor({ config }: { config: PricingConfig }) {
  const toast = useToast();
  const [c, setC] = useState(config);
  const set = (p: Partial<PricingConfig>) => setC((x) => ({ ...x, ...p }));
  const save = async () => { const e = await savePricingConfig(c); if (e) toast.error(e); else toast.success("Config guardada"); };
  return (
