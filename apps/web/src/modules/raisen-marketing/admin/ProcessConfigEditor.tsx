...
import { saveProcessConfig } from "@raisen-marketing/infrastructure/marketing-process.repository";
import type { ProcessConfig } from "@raisen-marketing/data/process.types";

const F = "w-full rounded-lg border border-border bg-background p-2 text-sm text-foreground";

// Editor del encabezado de la sección Proceso (eyebrow + título ES/EN).
export function ProcessConfigEditor({ config }: { config: ProcessConfig }) {
  const toast = useToast();
  const [c, setC] = useState(config);
  const set = (p: Partial<ProcessConfig>) => setC((x) => ({ ...x, ...p }));
  const save = async () => { const e = await saveProcessConfig(c); if (e) toast.error(e); else toast.success("Config guardada"); };
  return (
