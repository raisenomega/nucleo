...
import { saveFeaturesConfig } from "@raisen-marketing/infrastructure/marketing-features.repository";
import type { FeaturesConfig } from "@raisen-marketing/data/feature.types";

const F = "w-full rounded-lg border border-border bg-background p-2 text-sm text-foreground";

// Editor del encabezado de la sección Features (eyebrow + título ES/EN).
export function FeaturesConfigEditor({ config }: { config: FeaturesConfig }) {
  const toast = useToast();
  const [c, setC] = useState(config);
  const set = (p: Partial<FeaturesConfig>) => setC((x) => ({ ...x, ...p }));
  const save = async () => { const e = await saveFeaturesConfig(c); if (e) toast.error(e); else toast.success("Config guardada"); };
  return (
