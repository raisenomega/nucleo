...
import { saveTestimonialsConfig } from "@raisen-marketing/infrastructure/marketing-testimonials.repository";
import type { TestimonialsConfig } from "@raisen-marketing/data/testimonial.types";

const F = "w-full rounded-lg border border-border bg-background p-2 text-sm text-foreground";

// Editor del encabezado de la sección Testimonios (eyebrow + título ES/EN).
export function TestimonialsConfigEditor({ config }: { config: TestimonialsConfig }) {
  const toast = useToast();
  const [c, setC] = useState(config);
  const set = (p: Partial<TestimonialsConfig>) => setC((x) => ({ ...x, ...p }));
  const save = async () => { const e = await saveTestimonialsConfig(c); if (e) toast.error(e); else toast.success("Config guardada"); };
  return (
