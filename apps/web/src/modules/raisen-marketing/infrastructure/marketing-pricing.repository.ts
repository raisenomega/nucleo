...
const toCfg = (r: Record<string, unknown>): PricingConfig => ({ id: r.id as string, eyebrowEs: r.eyebrow_es as string, eyebrowEn: r.eyebrow_en as string, titleEs: r.title_es as string, titleEn: r.title_en as string });
...
export async function savePricingConfig(c: PricingConfig): Promise<string | null> {
  const { error } = await supabase.from("marketing_pricing_config").update({ eyebrow_es: c.eyebrowEs, eyebrow_en: c.eyebrowEn, title_es: c.titleEs, title_en: c.titleEn }).eq("id", c.id);
  return error ? error.message : null;
}
