...
export async function submitLead(s: LeadSubmit): Promise<{ ok: boolean; message?: string }> {
  ...
  const payload = { customer_name: s.customerName, customer_email: s.customerEmail, customer_phone: s.customerPhone, company: s.company, message: s.message, lead_type: s.leadType, source_url: window.location.href, utm_source: p.get("utm_source"), utm_medium: p.get("utm_medium"), utm_campaign: p.get("utm_campaign") };
