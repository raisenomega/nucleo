...
export function OrderSubmitBar({ form, totals, promoContext, busy, redirecting, useStripe, isSub, pm, locale, blocked, offerHook, offerAccepted, onSubmit, onClose }: {
  form: OrderForm; totals: Totals; promoContext?: PromoHeaderCtx; busy: boolean; redirecting: boolean;
  useStripe: boolean; isSub: boolean; pm: string; locale: string; blocked?: boolean; offerHook?: number | null; offerAccepted?: boolean; onSubmit: () => void; onClose: () => void;
}) {
  const { t } = useI18n();
  ...
  return (
    <div className={bar}>
      {promoContext?.summaryLine ? <PromoOrderSummary promo={promoContext} total={totals.total} />
        : form.showSummary ? <OrderDynamicSummary totals={totals} title={t("opSummaryTitle")} footer={locale === "en" ? form.summaryFooterEn : form.summaryFooterEs} recurring={isSub ? totals.total : null} hookPrice={offerHook} offerAccepted={offerAccepted} />
