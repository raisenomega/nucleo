...
export interface Totals { subtotal: number; tax: number; shipping: number; total: number; unitPrice: number | null; unitLabelEs: string | null; unitLabelEn: string | null }
...
export function useOrderPricing(item: OrderItem, values: Record<string, unknown>, coupon: string | null): Totals {
  const [totals, setTotals] = useState<Totals>({ subtotal: item.basePrice, tax: 0, shipping: 0, total: item.basePrice, unitPrice: null, unitLabelEs: null, unitLabelEn: null });
  ...
  useEffect(() => {
    const h = setTimeout(() => {
      void supabase.rpc("_public_preview_price", { _hostname: window.location.hostname,
        ...
        .then(({ data }) => {
          const d = data as { subtotal: number; tax: number; shipping: number; total: number; breakdown?: { steps?: Step[] } } | null;
          if (!d) return;
          const st = d.breakdown?.steps?.find((s) => s.rule === "matrix_1d");
          setTotals({ subtotal: Number(d.subtotal) || 0, tax: Number(d.tax) || 0, shipping: Number(d.shipping) || 0, total: Number(d.total) || 0,
