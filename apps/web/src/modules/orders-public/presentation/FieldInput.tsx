...
import { DisclaimerField } from "@orders-public/presentation/DisclaimerField";
...
export function FieldInput({ field, value, onChange }: { field: OrderFormField; value: unknown; onChange: (v: unknown) => void }) {
  ...
  if (field.kind === "disclaimer") return <DisclaimerField field={field} />;
  const label = (locale === "en" ? field.labelEn : field.labelEs) + (field.required ? " *" : "");
  const optLabel = (o: FieldOption) => (locale === "en" ? o.label_en : o.label_es) + (o.price_display ? ` · ${o.price_display}` : "");
  const helper = (locale === "en" ? field.validation.helper_en : field.validation.helper_es) as string | undefined;
  const ph = (locale === "en" ? field.placeholderEn : field.placeholderEs) ?? undefined;
  const cls = "w-full rounded-lg border border-border bg-background p-2 text-sm";
  const v = value ?? "";
  const helpEl = helper ? <p className="mt-1 text-xs text-muted-foreground">{helper}</p> : null;
  const wrap = (input: ReactNode) => (
    <label className="block"><span className="mb-1 block text-sm font-medium text-foreground">{label}</span>{input}{helpEl}</label>
  );
  ...
  if (field.kind === "textarea") return wrap(<textarea value={v as string} placeholder={ph} onChange={(e) => onChange(e.target.value)} rows={3} className={cls} />);
  ...
  if (field.kind === "radio") return (
    <div><span className="mb-1 block text-sm font-medium text-foreground">{label}</span>
      <div className="flex flex-wrap gap-4">{field.options.map((o) => <label key={o.value} className="flex items-center gap-1 text-sm text-foreground"><input type="radio" checked={v === o.value} onChange={() => onChange(o.value)} />{optLabel(o)}</label>)}</div>{helpEl}</div>
  );
  if (field.kind === "number") return wrap(<input type="number" value={v as number} placeholder={ph} onChange={(e) => onChange(e.target.value === "" ? "" : Number(e.target.value))} className={cls} />);
  const type = field.kind === "email" ? "email" : field.kind === "tel" ? "tel" : "text";
  return wrap(<input type={type} value={v as string} placeholder={ph} onChange={(e) => onChange(e.target.value)} className={cls} />);
}
