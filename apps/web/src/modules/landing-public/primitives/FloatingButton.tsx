...
export interface FloatingButtonProps {
  ...
  href?: string; onClick?: () => void; disabled?: boolean; className?: string;
}
const SIZE = { sm: "px-4 py-2 text-sm", md: "px-6 py-3", lg: "px-8 py-4 text-lg" };

export function FloatingButton({ children, variant = "primary", size = "md", href, onClick, disabled, className = "" }: FloatingButtonProps) {
  ...
  return <m.button type="button" onClick={onClick} disabled={disabled} style={{ background: bg }} className={`${cls} disabled:opacity-50`} {...anim}>{children}</m.button>;
}
