...
type Row = { kind: string; id: string; slug: string; name: string; base_price: number | string; hero: Record<string, unknown> };
...
function toSection(r: Row): LandingHeroSection {
  ...
  return {
    ...
    linkTargetSlug: (h.link_target_slug as string) ?? null, linkLabelEs: str(h, "link_label_es"), linkLabelEn: str(h, "link_label_en"),
  };
}
