...
export const ACCESS_MODULES: { key: string; label: string; crud: Perm[]; note: string }[] = [
  ...
  { key: "sales", label: "Órdenes de venta", crud: ALL, note: "SO → conduce → factura (ceo/coo/almacén)" },
  { key: "orders", label: "Órdenes", crud: [...ALL], note: "órdenes web (ceo/coo)" },
  { key: "customers", label: "Portal cliente", crud: ALL, note: "clientes del portal (ceo/coo)" },
...
export const ROLE_DEFAULTS: Record<AppRole, ModuleAccess> = {
  ...
  operaciones: { dashboard: V, expenses: { view: true, create: true }, inventory: { view: true, edit: true },
    assets: { view: true, edit: true }, sales: { view: true, edit: true }, accounts_receivable: { view: true }, reports: { view: true }, evaluations: { view: true, create: true },
