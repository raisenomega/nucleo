...
export function PayrollForm({ employees, payCats, initial, onSubmit, onCancel }: {
  employees: Emp[]; payCats: Cat[];
  initial?: PayrollFormData;
  onSubmit: (d: PayrollFormData) => void;
  onCancel: () => void;
}) {
  const { t } = useI18n();
  const { session } = useSession();
  const [f, setF] = useState<PayrollFormData>(
    initial ?? { employeeId: "", amount: 0, period: "", paymentMethodId: "", date: "", notes: "", evidenceUrls: [] },
  );
