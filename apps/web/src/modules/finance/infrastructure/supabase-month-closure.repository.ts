...
import type { IMonthClosureRepository, MonthClosure, MonthTotals, RepoResult } from "@finance/domain/month-closure.types";
...
export const supabaseMonthClosureRepository: IMonthClosureRepository = {
  async listClosures() {
    const { data } = await supabase.from("month_closures")
      .select("period_year, period_month, total_income, total_expenses, total_payroll, total_extraordinary, net_balance, closed_by, closed_at")
