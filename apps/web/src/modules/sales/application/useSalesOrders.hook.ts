...
export function useSalesOrders(repo: ISalesOrderRepository) {
  ...
  const createFromQuote = useCallback(async (qid: string) => { const r = await repo.createFromQuote(qid); await load(); return r; }, [repo, load]);
  const invoice = useCallback(async (id: string) => { const r = await repo.invoiceFromOrder(id); await load(); return r; }, [repo, load]);
