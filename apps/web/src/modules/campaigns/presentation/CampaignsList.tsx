...
// Lista de campañas del scope (superadmin=sentinela / tenant=las suyas — el aislamiento lo hace list_campaign_pages).
// `nav` provee el dominio a mostrar y las rutas del editor/inbox (mismos componentes para ambos scopes).
        ...
        <div className="space-y-2">{rows.map((r) => (
          <div key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-border bg-card p-3">
            <div className="min-w-0">
              <button type="button" onClick={() => nav.toEditor(r.id)} className="font-bold text-foreground hover:underline">{r.name}</button>
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              <a href={`https://${nav.host}/c/${r.slug}?preview=true`} target="_blank" rel="noreferrer" className="rounded-lg border border-border p-1.5" title="Ver / vista previa"><Eye className="h-4 w-4" /></a>
