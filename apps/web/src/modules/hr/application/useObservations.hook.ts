...
export function useObservations(repo: IObservationRepository) {
  ...
  const save = useCallback(async (employeeId: string, category: ObsCategory, notes: string, followUp: string | null, signature: string | null) => {
    const r = await repo.save(employeeId, category, notes, followUp, signature); if (r.ok) await load(); return r;
  }, [repo, load]);
