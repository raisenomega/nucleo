...
export function RecruitmentPage() {
  ...
  const [tab, setTab] = useState<"positions" | "openings">("positions");
  ...
  return (
    <div className="space-y-6 p-4 md:p-8">
      <div className="flex items-center justify-between gap-4">
        ...
        <button type="button" onClick={() => (tab === "positions" ? setPos({ open: true }) : setOp({ open: true }))}
      ...
      {m.loading ? <p className="text-sm text-muted-foreground">{t("noData")}</p> : tab === "positions" ? (
